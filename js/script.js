/* Botón final de búsqueda realizado con Vue.component */
Vue.component('button-search', {
  template: '<button v-on:click="mensajeFinal" class="btn btn-primary" id="resultado" type="button">Buscar</button>',
  methods: {
...
var mensajeFinal = new Vue({ el: '#buscar' })

/* Vue mostrando un string final cuando ya se ha realizado la reserva */
var resultFinal = new Vue({
  el: '#result',
  data: {
    bool: false
  }
...
/* Vue imprimiendo los 'Número de Noches' populares */
var popularNights = new Vue({
  el: '#populares',
  data: {
    noches: [
      { night: '4 Noches' },
...
/* Vue con los mensajes en el campo 'Nombre del Hotel' y 'Número de Noches' */
new Vue({
  el: '#contenido',
  data: {
     selected: "",
...
function habitacionExtra() {
  if (limitador < 3) {
    ...
    alert("No es posible añadir más habitaciones");
  }
